    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div class="bg-blue-50 rounded-lg p-6 mb-8 flex items-center justify-between">
            <div class="flex items-center">
                <div class="bg-blue-500 text-white rounded-full w-12 h-12 flex items-center justify-center text-lg font-bold mr-4">
                    {{ strtoupper(substr($user['full_name'] ?? $user['email'] ?? 'A', 0, 2)) }}
                </div>
                <div>
                    <h2 class="text-xl font-semibold text-gray-900">Welcome back, {{ $user['full_name'] ?? ($user['email'] ?? 'Admin') }}!</h2>
                    <p class="text-gray-600">Administrator Dashboard - Full system access</p>
                </div>
            </div>
            <div class="text-right">
                <p class="text-sm text-gray-500">Last login</p>
                <p class="text-sm font-medium text-gray-900">{{ date('M j, Y g:i A') }}</p>
            </div>
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
            ...
            <div class="bg-white rounded-lg shadow-sm p-6 hover:shadow-md transition-shadow">
                ...
                <div class="mt-4 space-y-2">
                    <a href="{{ route('admin.orders.index') }}" class="block text-blue-600 hover:text-blue-800 font-medium text-sm">
                        <i class="bi bi-list-ul me-1"></i>All Orders
                    </a>
                    <a href="{{ route('admin.orders.index', ['status' => 'pending']) }}" class="block text-yellow-600 hover:text-yellow-800 font-medium text-sm">
            ...
            <div class="bg-white rounded-lg shadow-sm p-6 hover:shadow-md transition-shadow">
                ...
                <div class="mt-4 space-y-2">
                    <a href="{{ route('admin.products.create') }}" class="block text-green-600 hover:text-green-800 font-medium text-sm">
                        <i class="bi bi-plus-circle me-1"></i>Add Product
                    </a>
                    <a href="{{ route('admin.products.index') }}" class="block text-green-600 hover:text-green-800 font-medium text-sm">
            ...
            <div class="bg-white rounded-lg shadow-sm p-6 hover:shadow-md transition-shadow">
                ...
                <div class="mt-4 space-y-2">
                    <a href="{{ route('admin.users.index') }}" class="block text-purple-600 hover:text-purple-800 font-medium text-sm">
                        <i class="bi bi-people me-1"></i>All Users
                    </a>
                    <a href="{{ route('admin.users.roles') }}" class="block text-purple-600 hover:text-purple-800 font-medium text-sm">
            ...
            <div class="bg-white rounded-lg shadow-sm p-6 hover:shadow-md transition-shadow">
                ...
                <div class="mt-4 space-y-2">
                    <a href="{{ route('admin.payments.index') }}" class="block text-orange-600 hover:text-orange-800 font-medium text-sm">
                        <i class="bi bi-credit-card me-1"></i>Payments
                    </a>
                    <a href="{{ route('admin.reports.sales') }}" class="block text-orange-600 hover:text-orange-800 font-medium text-sm">
        ...
        <div class="grid grid-cols-1 md:grid-cols-4 gap-6 mb-6">
            ...
            <div class="bg-white rounded-lg shadow-sm p-6">
                ...
                <div class="mt-4">
                    <span class="text-yellow-600 text-sm font-medium">{{ $stats['pending_orders'] ?? 0 }} need attention</span>
                </div>
            </div>
            ...
            <div class="bg-white rounded-lg shadow-sm p-6">
                ...
                <div class="mt-4">
                    <span class="text-green-600 text-sm font-medium">{{ $stats['active_products'] ?? 0 }} active</span>
                </div>
            </div>
            ...
            <div class="bg-white rounded-lg shadow-sm p-6">
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <p class="text-2xl font-bold text-gray-900">TZS {{ number_format($stats['total_revenue'] ?? 0, 2) }}</p>
                    </div>
                    ...
                    @if(count($recentOrders ?? []) > 0)
                        <div class="space-y-4">
                            @foreach(array_slice($recentOrders, 0, 5) as $order)
                                <div class="flex items-center justify-between">
                                    <div>
                                        <p class="text-sm font-medium text-gray-900">#{{ $order['order_number'] ?? 'N/A' }}</p>
                                        <p class="text-xs text-gray-500">{{ $order['customer_name'] ?? 'Unknown Customer' }}</p>
                                    </div>
                                    <div class="text-right">
                                        <p class="text-sm font-medium text-gray-900">TZS {{ number_format($order['total_amount'] ?? 0, 2) }}</p>
