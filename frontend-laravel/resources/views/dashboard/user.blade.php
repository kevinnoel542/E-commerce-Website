...
<body class="bg-gray-50">
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div class="bg-blue-50 rounded-lg p-6 mb-8 flex items-center">
            ...
            <div>
                <h2 class="text-xl font-semibold text-gray-900">Welcome back, {{ $user['full_name'] ?? ($user['email'] ?? 'User') }}!</h2>
        ...
        <div class="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
            ...
            <div class="bg-white rounded-lg shadow-sm p-6">
                ...
                <div class="mt-4">
                    <span class="text-yellow-600 text-sm font-medium">{{ $pendingPayments ?? 0 }} need attention</span>
                </div>
            </div>
            ...
            <div class="bg-white rounded-lg shadow-sm p-6">
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <p class="text-2xl font-bold text-gray-900">{{ $addressCount ?? 1 }}</p>
                    </div>
                ...
                <div class="mt-4">
                    <a href="{{ route('profile.edit') }}" class="text-green-600 hover:text-green-800 text-sm font-medium">
                        Manage Addresses →
                    </a>
                </div>
            </div>
            ...
            <div class="bg-white rounded-lg shadow-sm p-6">
                ...
                <div class="mt-4">
                    <a href="{{ route('wishlist.index') }}" class="text-purple-600 hover:text-purple-800 text-sm font-medium">
                        View Wishlist →
                    </a>
                </div>
            </div>
                            ...
                            <tbody>
                                @foreach(array_slice($orders, 0, 5) as $order)
                                    <tr class="border-b border-gray-100 hover:bg-gray-50">
                                        <td class="py-3 px-4">
                                            <span class="font-medium text-gray-900">#{{ $order['order_number'] ?? 'N/A' }}</span>
                                        </td>
                                        <td class="py-3 px-4">
                                            <span class="text-gray-600">{{ date('M j, Y', strtotime($order['created_at'] ?? 'now')) }}</span>
                                        </td>
                                        <td class="py-3 px-4">
                                            <span class="text-gray-600">{{ count($order['items'] ?? []) }} items</span>
                                        </td>
                                        <td class="py-3 px-4">
                                            <span class="font-medium text-gray-900">TZS {{ number_format($order['total_amount'] ?? 0, 2) }}</span>
                                        </td>
                                        <td class="py-3 px-4">
