                ...
                <nav class="flex space-x-8 px-6" aria-label="Tabs">
                    <a href="{{ route('admin.orders.index') }}" 
                       class="py-4 px-1 border-b-2 font-medium text-sm {{ !request('status') ? 'border-blue-500 text-blue-600' : 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300' }}">
                        All Orders
                        <span class="ml-2 bg-gray-100 text-gray-900 py-0.5 px-2.5 rounded-full text-xs">{{ $pagination['total'] ?? 0 }}</span>
                    </a>
                    <a href="{{ route('admin.orders.index', ['status' => 'pending']) }}" 
                       class="py-4 px-1 border-b-2 font-medium text-sm {{ request('status') === 'pending' ? 'border-yellow-500 text-yellow-600' : 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300' }}">
                        Pending
                        <span class="ml-2 bg-yellow-100 text-yellow-900 py-0.5 px-2.5 rounded-full text-xs">{{ collect($orders ?? [])->where('status', 'pending')->count() }}</span>
                    </a>
                    <a href="{{ route('admin.orders.index', ['status' => 'confirmed']) }}" 
                       class="py-4 px-1 border-b-2 font-medium text-sm {{ request('status') === 'confirmed' ? 'border-blue-500 text-blue-600' : 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300' }}">
                        Confirmed
                        <span class="ml-2 bg-blue-100 text-blue-900 py-0.5 px-2.5 rounded-full text-xs">{{ collect($orders ?? [])->where('status', 'confirmed')->count() }}</span>
                    </a>
                    <a href="{{ route('admin.orders.index', ['status' => 'shipped']) }}" 
                       class="py-4 px-1 border-b-2 font-medium text-sm {{ request('status') === 'shipped' ? 'border-purple-500 text-purple-600' : 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300' }}">
                        Shipped
                        <span class="ml-2 bg-purple-100 text-purple-900 py-0.5 px-2.5 rounded-full text-xs">{{ collect($orders ?? [])->where('status', 'shipped')->count() }}</span>
                    </a>
                    <a href="{{ route('admin.orders.index', ['status' => 'delivered']) }}" 
                       class="py-4 px-1 border-b-2 font-medium text-sm {{ request('status') === 'delivered' ? 'border-green-500 text-green-600' : 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300' }}">
                        Delivered
                        <span class="ml-2 bg-green-100 text-green-900 py-0.5 px-2.5 rounded-full text-xs">{{ collect($orders ?? [])->where('status', 'delivered')->count() }}</span>
                    </a>
                </nav>
            </div>
        </div>
                            ...
                            @foreach($orders as $order)
                                <tr class="hover:bg-gray-50">
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                                        {{ date('M j, Y', strtotime($order['created_at'] ?? 'now')) }}
                                        <div class="text-xs text-gray-500">{{ date('g:i A', strtotime($order['created_at'] ?? 'now')) }}</div>
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                                        {{ count($order['items'] ?? []) }} items
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">
                                        TZS {{ number_format($order['total_amount'] ?? 0, 2) }}
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap">
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                        <div class="flex items-center space-x-2">
                                            <a href="{{ route('admin.orders.show', $order['id'] ?? 1) }}" 
                        ...
                        <div class="hidden sm:flex-1 sm:flex sm:items-center sm:justify-between">
                            <div>
                                <p class="text-sm text-gray-700">
                                    Showing <span class="font-medium">{{ (($pagination['current_page'] ?? 1) - 1) * ($pagination['per_page'] ?? 20) + 1 }}</span>
                                    to <span class="font-medium">{{ min(($pagination['current_page'] ?? 1) * ($pagination['per_page'] ?? 20), $pagination['total'] ?? 0) }}</span>
